...
class TFBaseModel(object):
    def __init__(
        ...
        self.chosen_optimizer = chosen_optimizer
    ...
    def fit(self):
        with self.session.as_default():
            ...
            train_generator = self.reader.train_batch_generator(self.batch_size_val)
            val_generator = self.reader.val_batch_generator(self.num_validation_batches*self.batch_size_val)
            ...
            while step < self.num_training_steps:
                ...
                val_feed_dict.update({self.learning_rate_var: self.rate_learning})
                ...
                train_feed_dict.update({self.learning_rate_var: self.rate_learning})
                if hasattr(self, 'keep_prob'):
    ...
    def update_parameters(self, loss):
        ...
        chosen_optimizer = self.get_optimizer(self.learning_rate_var)
        grads = chosen_optimizer.compute_gradients(loss)
        clipped = [(tf.clip_by_value(g, -self.grad_clip, self.grad_clip), v_) for g, v_ in grads]

        step = chosen_optimizer.apply_gradients(clipped, global_step=self.global_step)
    ...
    def get_optimizer(self, rate_learning):
        if self.chosen_optimizer == 'adam':
            return tf.train.AdamOptimizer(rate_learning)
        elif self.chosen_optimizer == 'gd':
            return tf.train.GradientDescentOptimizer(rate_learning)
        elif self.chosen_optimizer == 'rms':
            return tf.train.RMSPropOptimizer(rate_learning, decay=0.95, momentum=0.9)
        else:
            assert False, 'optimizer must be adam, gd, or rms'

    def graph_init(self):
        with tf.Graph().as_default() as graph:
            self.mavg = tf.train.ExponentialMovingAverage(decay=0.995)
            self.stepglobal = tf.Variable(0, trainable=False)
